...
def evaluate_acc(model, dataloader, losscriterion, device):
    ...
    with torch.no_grad():
        for ctr, data in enumerate(dataloader):
            ...
            _, preds = torch.max(cpuout, 1)  # values, and indices
            labels = labels.float()
...
def train_model_nocv_sizes(dataloader_train, dataloader_test, model, losscriterion, optimizer, scheduler, num_epochs,
                           device):
    best_measure = 0
    best_epoch = -1
...
def runstuff_fromscratch():
    ...
    basepath = '/work/jamesbc/data/flower_data/'
    ...
    accuracy, testloss = evaluate_acc(model=model, dataloader=dataloaders['test'], losscriterion=criterion,
                                      device=device)
